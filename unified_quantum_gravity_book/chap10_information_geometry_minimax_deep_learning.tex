...
Generalizing the obstacle minimax framework of Chapter 7 \cite{silvafilho2026minimax, dubins1957}, the optimal training trajectory is the regular path $\gamma: [0, L] \to \Theta \setminus \mathcal{O}_{\mathrm{sing}}$ with $\gamma(0) = \theta_0$ and $\gamma(L) \in \Sigma^*$ that minimizes the peak covariant curvature:
\begin{equation}
...
Therefore, the minimax curvature $\kappa^*_{\mathrm{info}}$ must not be applied to the microscopic parameter path, but to the macroscopic \textbf{2-Wasserstein metric} of the probability distribution over parameters $\rho(\theta, t) \in \mathcal{P}_2(\Theta)$. 
...
\begin{enumerate}
    ...
    \begin{equation}
    \E_{\theta \sim \rho^*} \left[ \Loss_{\mathrm{true}}(\theta) - \hat{\Loss}_{\mathrm{train}}(\theta) \right] \le \sqrt{ \frac{D \ln\left(1 + \frac{L^2 (\kappa^*_{\mathrm{info}})^2}{2\sigma^2}\right) + \ln(2/\delta)}{2 N} }
    \end{equation}
\end{enumerate}
\end{theorem}
...
Translations of Mathematical Monographs, vol. 191, AMS, 2000, \href{https://doi.org/10.1090/mmono/191}{doi:10.1090/mmono/191}.
