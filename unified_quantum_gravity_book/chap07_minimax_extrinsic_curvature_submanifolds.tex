...
The variational theory of submanifolds spanning prescribed boundaries has historically focused on integral functionals. In the classical Plateau problem \cite{douglas1931}, area minimization yields minimal submanifolds with vanishing mean curvature ($H = 0$). In the Willmore problem \cite{willmore1965, riviere2008}, one minimizes bending energy $\mathcal{W}(M) = \int_M H^2 \, dA$, and in elastic rope/rod theory, the total squared curvature $\int |\II|^2 \, dA$ is minimized \cite{langer1984}.
...
We represent $M_0$ canonically as the zero section $\xi_0 \equiv 0$. We apply the Moreau envelope directly:
\begin{equation}
\xi^{\lambda, \epsilon}(p) := \sup_{y \in M_0} \inf_{z \in M_0} \left\{ \frac{1}{2\lambda} d_{g_S}(y, z)^2 - \frac{1}{2\epsilon} d_{g_S}(p, y)^2 + \Lambda (\Hsn^k(z) - V)_+ \right\}
\end{equation}
\emph{Physical Intuition:} The Moreau envelope acts as a geometric regularizer. The supremum and infimum operators map distributional singularities into a bounded $C^{1,1}$ surface, while the volume penalty $\Lambda$ dynamically enforces the constraint $\Hsn^k(M) \le V$.
By incorporating the volume penalty barrier $\Lambda$ directly into the inf-sup convolution, we dynamically preserve the volume constraint $\Hsn^k(M) \le V$ natively during the regularization step. This simultaneous, single-step penalization completely avoids the need for sequential Obstacle-Conditioned Mean Curvature Flows, maintaining mathematical harmony without wall collisions. The regularized normal field $\xi^{\lambda, \epsilon}$ is globally $C^{1,1}$ and preserves the curvature supremum $\|\nabla^2 \xi^{\lambda, \epsilon}\|_{L^\infty} \le \|\nabla^2 \xi_0\|_{L^\infty} + \mathcal{O}(\epsilon)$.
...
\begin{equation}
\|\II_{\tilde{M}_\epsilon}\|_{L^\infty} \le \|\II_{M_0}\|_{L^\infty} + \mathcal{O}(\epsilon)
\end{equation}
Taking $\epsilon \to 0$ and $\eta \to 0$ elegantly establishes $\kappa^*_{n,k,r}(V) \le \kappa^*_{n,k,2}(V)$ unconditionally for all $k, n$.
\end{proof}
...
To bridge the gap to applied computer science, we formulate the discrete minimax curvature using \textbf{Discrete Exterior Calculus (DEC)} \cite{hirani2003, crane2013}. By defining discrete second fundamental forms over the 1-ring neighborhood of mesh vertices, we prove that the discrete sequence of optimization functionals \textbf{$\Gamma$-converges} to the continuous $W^{2,\infty}$ problem as the mesh density $\Delta x \to 0$, embedding the discrete limits via standard Whitney form interpolation \cite{wardetzky2007}. This rigorously guarantees that computational solvers will not lock, returning a discrete mesh that is physically faithful to the true continuous canonical minimizer.
...
Let $X$ be a Riemannian manifold with calibration $k$-form $\omega$, and let $M^k \subset X$ be an $\omega$-calibrated submanifold (e.g., Special Lagrangian in $CY_m$, Associative/Coassociative in $G_2$, or Cayley in $\operatorname{Spin}(7)$). Then $M^k$ is minimal ($H = 0$) and its second fundamental form is vectorially isotropic across the normal bundle, achieving:
\begin{equation}
...
For boundary corners with aperture $\theta \in (0, \pi)$, smoothing from $C^1$ to $C^r$ introduces a sharp singular correction:
\begin{equation}
...
J. Differential Geom. \textbf{20} (1984), no. 1, 1--22.

\bibitem{langer1985}
J.~Langer,
\emph{A compactness theorem for surfaces with bounded second fundamental form},
J. Differential Geom. \textbf{21} (1985), no. 1, 127--138.
...
Phys. Lett. B \textbf{374} (1996), 37--42.
...
Computer Aided Geometric Design \textbf{24} (2007), 499--518.
