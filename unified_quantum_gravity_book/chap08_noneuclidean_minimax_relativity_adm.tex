...
\begin{equation}
\kappa^*_{\Hyp^n} = \sqrt{\left(\frac{2}{w}\right)^2 - c^2} < \kappa^*_{\R^n} = \frac{2}{w}
\end{equation}
...
$\Hyp^n(-c^2)$ & Curve & $1$ & $n-1$ & Horocycle & Escape in Hyperbolic Space & $\sqrt{(2/w)^2 - c^2}$ \\
...
$\text{AdS}_5 \times \Sph^5$ & 4-Brane & $5$ & $5$ & Holographic Wilson Surface & AdS/CFT confinement loop & $\frac{1}{R_{\text{AdS}}}$ \\
