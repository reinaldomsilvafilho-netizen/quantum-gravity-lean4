...
\end{equation}
where $r_m = m+3$ is the harmonic renormalization factor for the Laplacian (combining the cell count $m+1$ with the resistance scaling $\rho = \frac{m+3}{m+1}$, so $r_m = (m+1)\rho = m+3$; for $m=2$, $r_2 = 5$).

In this work, expanding the continuous simplicial framework established in our foundational series \cite{silvafilho2026simplex, silvafilho2026waves, silvafilho2026inter}, we demonstrate that the continuous meromorphic continuation of the Pascal simplex via Euler's Gamma function $\Gamma(z)$ provides a smooth, non-local analytical bridge to fractal analysis and multifractal thermodynamics.
...
\begin{equation}
\mathcal{E}_k(u, u) = (m+3)^k \sum_{|w|=k} \frac{1}{2(\alpha 2^{-k})^2 I_m(\alpha 2^{-k})} \int_{F_w(K)} d\mathbf{x} \int_{\Delta_{m-1}(\alpha 2^{-k})} \binom{\alpha 2^{-k}}{\mathbf{y}} |u(\mathbf{x}+\mathbf{y}) - u(\mathbf{x})|^2 d\mathbf{y}.
\end{equation}
As $k \to \infty$, the integral localizes along the vertex junctions of the IFS cells. The weights converge to the harmonic decimation factors $r_m = m+3$, establishing the $\Gamma$-convergence of the quadratic forms $\mathcal{E}_k \xrightarrow{\Gamma} \mathcal{E}_{\text{Kigami}}$ on the domain $\mathcal{D} = \{u \in C(K) : \mathcal{E}_{\text{Kigami}}(u, u) < \infty\}$. By the Trotter--Kato theorem, $\Gamma$-convergence of closed Dirichlet forms implies strong resolvent convergence of the associated self-adjoint generators.
\end{proof}
...
\begin{enumerate}
    ...
    For $q \ne 1$, shifting by the baseline ground-state free energy $\tau(1) = -\sigma_0^2/2$:
    \begin{equation}
    D_q \coloneqq \frac{\tau(q) - \tau(1)}{q-1} = \ln 2 - \frac{\sigma_0^2}{2}\frac{q^2 - 1}{q-1} = \ln 2 - \frac{\sigma_0^2}{2}(q+1).
    \end{equation}
    In the limit $q \to 1$, the information dimension is given via l'H\^opital's rule by
    \begin{equation}
    D_1 \coloneqq \lim_{q \to 1} \frac{\tau(q) - \tau(1)}{q-1} = \tau'(1) = \ln 2 - \sigma_0^2 = \ln 2 - \frac{1}{2},
    \end{equation}
...
\begin{theorem}[Box-Counting Dimension of the Modulo-2 Simplicial Support]
...
\end{equation}
Let $\mathcal{S} \coloneqq \overline{ \lim_{k\to\infty} 2^{-k} \{(n, m) \in \mathbb{N}^2 : \binom{n}{m} \equiv 1 \pmod 2 \} }$ denote the Hausdorff scaling limit of the non-vanishing multinomial support modulo $2$, which tiles the nodal domains across dyadic chambers. The Minkowski box-counting dimension of $\mathcal{S}$ satisfies:
\begin{equation}
\dim_{\text{box}}(\mathcal{S}) = \frac{\ln 3}{\ln 2} \equiv d_H(\text{Sierpi\'nski Gasket}).
\end{equation}
\end{theorem}

\begin{proof}
In the reflection domain, the integer nodal crossings partition $\mathbb{R}^2$ into dyadic triangular chambers. By Lucas' Theorem \cite{lucas1878}, the parity of $\binom{n}{m} \pmod 2$ is non-vanishing if and only if the binary expansion of $m$ is a sub-digit expansion of $n$. Partitioning the region $[0, 2^k]^2$ (or $[-2^k, 0]^2$) into dyadic triangular cells of side length $2^{-j}$, the number of active non-vanishing chambers at scale $2^{-j}$ satisfies the exact ternary branching recurrence:
\begin{equation}
N(2^{-j}) = 3^j.
\end{equation}
By definition of the Minkowski box-counting dimension:
\begin{equation}
\dim_{\text{box}}(\mathcal{S}) = \lim_{j \to \infty} \frac{\ln N(2^{-j})}{-\ln(2^{-j})} = \lim_{j \to \infty} \frac{\ln 3^j}{\ln 2^j} = \frac{\ln 3}{\ln 2},
\end{equation}
recovering identically the Hausdorff dimension of the Sierpi\'nski gasket.
\end{proof}
...
\bibitem{silvafilho2026simplex} R. M. Silva-Filho, \emph{Analytic Continuation of Pascal's Simplex: Continuous Multinomial Integrals, Polytope Boundary Recurrences, and Simplicial Fractional Calculus}, Treatise Chapter 03, 2026.
\bibitem{silvafilho2026waves} R. M. Silva-Filho, \emph{Nonlinear Simplicial Waves and Anomalous Porous Transport Induced by Beta-Kernel Fractional Laplacians}, Treatise Chapter 04, 2026.
\bibitem{silvafilho2026inter} R. M. Silva-Filho, \emph{Inter-Dimensional Simplicial Transforms, Fractional Boundary Traces, and Grassmannian Beta-Kernels}, Treatise Chapter 05, 2026.
...
\bibitem{barnes1900} E. W. Barnes, \emph{The Theory of the G-Function}, Quarterly Journal of Pure and Applied Mathematics, vol. 31, pp. 264--314, 1900.
\bibitem{falconer2014} K. Falconer, \emph{Fractal Geometry: Mathematical Foundations and Applications}, 3rd ed., John Wiley \& Sons, Chichester, 2014, \href{https://doi.org/10.1002/0470013850}{doi:10.1002/0470013850}.
