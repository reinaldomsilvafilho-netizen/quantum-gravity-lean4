...
\end{equation}
Functionally, the matrix represents the maximally rough spatial signal.
\end{example}
...
\end{theorem}
\begin{proof}
Since $W_A \in \BV((0,1)^2)$ by Theorem~\ref{thm:tv_matrix}, De Giorgi's structure theorem guarantees that for almost every $t \in \R$, $E_t$ is a Caccioppoli set of finite perimeter and the classical Federer Coarea Identity holds (see Federer~\cite{federer1969geometric}, Ambrosio, Fusco, and Pallara~\cite{ambrosio2000functions}, and Evans and Gariepy~\cite{evans2015measure}).
\end{proof}
...
Let $\bar{\mathcal{W}}$ be the space of symmetric graphons $W \colon [0, 1]^2 \to [0, 1]$, and let $\bar{\Pi}$ be the group of measure-preserving bijections. The quotient metric space $(\widetilde{\mathcal{W}}, \delta_\square) \coloneqq (\bar{\mathcal{W}} / \bar{\Pi}, \delta_\square)$ is \emph{compact}. Consequently, every sequence of symmetric matrices $\{A_n\}_{n=1}^\infty$ with entries uniformly bounded in a compact interval $[m, M]$ (under standard affine normalization $a_{ij} \mapsto \frac{a_{ij} - m}{M - m} \in [0, 1]$) has a subsequence whose step realizations $W_{A_{n_k}}$ converge in the cut metric to a limit graphon $W \in \widetilde{\mathcal{W}}$.
\end{theorem}
\begin{proof}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \end{equation}
    which unconditionally recover the Lim--Qi singular value equations~\cite{lim2005singular,qi2005eigenvalues} without diverging factor weights.
\end{enumerate}
\end{theorem}
\begin{proof}
...
Let $f_{\mathcal{A}_{b,h}}(x, y) \in H^2(\Tor^2)$ be the harmonic realization of the attention slice $\mathcal{A}_{b, h, :, :}$. If the network training loss includes the multilinear Sobolev penalty controlling the full $H^2(\Tor^2)$ norm:
\begin{align}
    \mathcal{R}_{\mathrm{Attn}}(\mathcal{A}) &\coloneqq \sum_{b=1}^B \sum_{h=1}^H \iint_{\Tor^2} \left( |f_{\mathcal{A}_{b,h}}|^2 + \|\nabla f_{\mathcal{A}_{b,h}}\|_2^2 + \lambda |\Delta f_{\mathcal{A}_{b,h}}|^2 \right) \dif x \dif y \notag \\
    &= \sum_{b=1}^B \sum_{h=1}^H \|f_{\mathcal{A}_{b,h}}\|_{H^2(\Tor^2)}^2,
\end{align}
...
\end{theorem}
\begin{proof}
In dimension 2, the Sobolev embedding theorem guarantees that the Sobolev space $H^2(\Tor^2)$ embeds continuously into the H\"older space $C^{0, \alpha}(\Tor^2)$ for all $\alpha \in (0, 1)$. By Morrey's inequality and elliptic Sobolev theory:
\begin{equation}
    \|f\|_{L^\infty(\Tor^2)} \le C_{\mathrm{Sobolev}} \|f\|_{H^2(\Tor^2)} = C_{\mathrm{Sobolev}} \left( \|f\|_{L^2}^2 + \|\nabla f\|_{L^2}^2 + \lambda \|\Delta f\|_{L^2}^2 \right)^{1/2}.
\end{equation}
Furthermore, because each row of the discrete attention matrix $\mathcal{A}_{b,h,:,:}$ is normalized via softmax ($\sum_{j=1}^N \mathcal{A}_{b,h,i,j} = 1$ with entries $\mathcal{A}_{b,h,i,j} \in [0,1]$), the $L^2$-norm $\|f_{\mathcal{A}}\|_{L^2} = \|\mathcal{A}\|_F \le \sqrt{N}$ is bounded by the stochastic nature of the attention weights. Combining the full $H^2$ penalty with the Sobolev embedding constant $C_{\mathrm{Sobolev}}$ yields the claimed bound on the supremum.
\end{proof}
...
\end{theorem}
\begin{proof}
By the Kac-Rice formula for random fields on Riemannian manifolds (see Adler and Taylor~\cite{adler2007random}), the expected number of critical points is given by:
\begin{equation}
...
\end{equation}
where $\nabla^2 f_{\mathcal{T}}(x)$ is the covariant Riemannian Hessian on $T_x \Sph^{n-1}$ and $p_{\nabla f_{\mathcal{T}}(x)}(0)$ is the marginal density of the gradient at zero. Under the spherical symmetry of the covariance kernel $K(x, y) = \frac{1}{d} (x \cdot y)^d$, the field $f_{\mathcal{T}}$ and its gradient are isotropic. Conditional on $\nabla f_{\mathcal{T}}(x) = 0$, the covariant Hessian splits into:
\begin{equation}
    \nabla^2 f_{\mathcal{T}}(x) = H_0 - (d-1) f_{\mathcal{T}}(x) I_{n-1},
\end{equation}
where $H_0$ is an $(n-1) \times (n-1)$ Gaussian Orthogonal Ensemble (GOE) matrix independent of $f_{\mathcal{T}}(x)$. By Wigner's semicircle law and the large deviations principle for the empirical spectral measure of GOE matrices, the expected absolute determinant scales asymptotically as $\exp(n \Sigma(d))$ where $\Sigma(d) = \frac{1}{2} \log(d-1) - \frac{d-2}{2(d-1)} > 0$ for $d \ge 3$ (Auffinger, Ben Arous, and C\'erny~\cite{auffinger2013random}, Theorem 1.1). Integrating across the sphere of volume $\operatorname{Vol}(\Sph^{n-1})$ and over the energy levels yields the exponential growth $\sim C(d) \exp(n \, \Theta(d))$.
\end{proof}
...
\end{equation}
Conditioning on $(u^{(2)}, \dots, u^{(k)})$, the function $u^{(1)} \mapsto \widehat{f}_{\mathcal{T}}(u^{(1)}, \dots, u^{(k)})$ is linear on $\Sph^{d-1}$ with Lipschitz constant $L_1 = \frac{1}{\sqrt{d}} \|w\|_2$, where $w_i \coloneqq \sum_{j_2, \dots, j_k} \mathcal{T}_{i, j_2 \dots j_k} u_{j_2}^{(2)} \dots u_{j_k}^{(k)}$. Since $\mathcal{T}$ is Gaussian, the coordinates $w_1, \dots, w_d$ are independent centered Gaussian variables with variance $\prod_{\alpha=2}^k \|u^{(\alpha)}\|_2^2 = 1$. Hence $w \sim \mathcal{N}(0, I_d)$ and $\|w\|_2^2 \sim \chi^2(d)$. By the Laurent--Massart tail bound for chi-squared distributions, $\mathbb{P}(\|w\|_2^2 \ge 2d) \le \exp(-d/8)$. On the typical event $\{\|w\|_2^2 \le 2d\}$, the Lipschitz constant satisfies $L_1 \le \sqrt{2}$. Applying the Herbst argument inductively across all $k$ spheres and unconditioning via the exponential chi-squared tail yields \eqref{eq:concentration_inequality}.
\end{proof}
