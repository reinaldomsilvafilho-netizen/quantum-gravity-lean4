...
\hypersetup{
  ...
  citecolor=Mahogany,
  urlcolor=TealBlue,
  pdfauthor={Functional Matrix Research Group},
...
Let $A \in \C^{n \times n}$. Let $\Tor^2 = [0, 2\pi)^2$ be the 2-torus. We associate $A$ with a trigonometric polynomial:
\begin{equation}
    \label{eq:fourier_realization}
    f_A(x_1, x_2) \coloneqq \frac{1}{n} \sum_{k_1, k_2 = 1}^n a_{k_1 k_2} e^{i (k_1 x_1 + k_2 x_2)}.
\end{equation}
By Parseval's identity, the $L^2(\Tor^2)$ norm satisfies $\|f_A\|_{L^2(\Tor^2)} = \|A\|_F$.
\end{construction}
...
Now consider the harmonic realization on $\Tor^2 = [0, 2\pi)^2$ given by \eqref{eq:fourier_realization}:
\begin{equation}
    f_A(x_1, x_2) = \frac{1}{n} \sum_{k_1, k_2 = 1}^n a_{k_1 k_2} e^{i (k_1 x_1 + k_2 x_2)}.
\end{equation}

The homogeneous Dirichlet energy is:
\begin{equation}
    \mathcal{E}(f) \coloneqq \|\nabla f\|_{L^2(\Tor^2)}^2 = \int_{\Tor^2} \|\nabla f(x_1, x_2)\|_2^2 \dif x_1 \dif x_2.
\end{equation}
...
\begin{equation}
    \partial_{x_1} f_A = \frac{1}{n} \sum_{k_1, k_2=1}^n (i k_1) a_{k_1 k_2} e^{i(k_1 x_1 + k_2 x_2)}, \quad
    \partial_{x_2} f_A = \frac{1}{n} \sum_{k_1, k_2=1}^n (i k_2) a_{k_1 k_2} e^{i(k_1 x_1 + k_2 x_2)}.
\end{equation}
...
\subsection{Asymptotic Graphons and the Cut-Norm Topology ($n \to \infty$)}
...
\end{theorem}
\begin{proof}
To prove (i), observe that for fixed unit vectors $u^{(\alpha)} \in \Sph^{d-1}$, the sum $X \coloneqq \sum \mathcal{T}_{j_1 \dots j_k} u_{j_1}^{(1)} \dots u_{j_k}^{(k)}$ is a sum of independent random variables with mean zero and variance:
\begin{equation}
...
The foundational operator positivity and multilinear algebra constructions utilized in this framework have been formally verified in the \textsc{Lean 4} interactive theorem prover (toolchain \texttt{v4.33.1}) with zero \texttt{sorry} statements at:
\begin{center}
