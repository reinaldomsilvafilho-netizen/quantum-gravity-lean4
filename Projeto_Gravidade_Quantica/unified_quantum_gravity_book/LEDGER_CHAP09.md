...
### OBL-C09-011: Quantitative Teardrop Loop Canonical Benchmark
- **Type:** Proposition / Computational Benchmark
- **Statement:** In the canonical obstacle corridor $\Omega_H$ (parameters $R_0 = 1.00$, $H = 1.62$), the immersed teardrop path ($\mathbf{w}=1$) achieves $\kappa^*_{\mathrm{Imm}} \approx 0.6173\ \text{m}^{-1}$ compared to $\kappa^*_{\mathrm{Jordan}} \approx 1.2500\ \text{m}^{-1}$, yielding a certified $\Delta\kappa_{\mathrm{rel}} = 1 - w_{\mathrm{eff}}/H \approx 50.62\%$ reduction in peak extrinsic curvature, numerically certifying the general Topological Curvature Gap Theorem ($\kappa^*_{\mathrm{emb}} > \kappa^*_{\mathrm{imm}}$).
- **Status:** `CERTIFIED`
