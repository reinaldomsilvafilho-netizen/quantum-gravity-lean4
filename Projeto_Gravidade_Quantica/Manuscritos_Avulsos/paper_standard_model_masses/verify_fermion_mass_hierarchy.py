...
def test_cp_violation_jarlskog():
    print_header("Battery 5: CP Violation & Dimensionless Jarlskog Invariant J_CP")
    
    alpha_s = 0.1180
    C_F = 4.0 / 3.0
    eps_qcd = C_F * alpha_s / (4.0 * np.pi)
    
    # 1. Delta_CP phase
    delta_CP_theo = np.pi / 3.0 + np.sqrt(3.0) * np.pi * eps_qcd  # rad
    delta_CP_deg = delta_CP_theo * 180.0 / np.pi
    
    # 2. CKM angles: s12 from geometric GST, s23 and s13 from PDG
    m_d, m_s = 4.67e-3, 93.4e-3  # GeV
    s12 = np.sqrt(m_d / m_s) * (1.0 + eps_qcd)
    c12 = np.sqrt(1.0 - s12**2)
    s23 = 0.0422  # PDG |V_cb|
    c23 = np.sqrt(1.0 - s23**2)
    s13 = 0.00369  # PDG |V_ub|
    c13 = np.sqrt(1.0 - s13**2)
    
    # 3. Standard dimensionless product formula
    J_CP_theo = c12 * c23 * (c13**2) * s12 * s23 * s13 * np.sin(delta_CP_theo)
    J_CP_exp = 3.08e-5
    
    print(f"  Theoretical CP Phase delta_CP:            {delta_CP_theo:.4f} rad ({delta_CP_deg:.2f} deg)")
    print(f"  Theoretical Jarlskog Invariant J_CP:      {J_CP_theo:.4e}")
    print(f"  Experimental Jarlskog Invariant J_CP:     {J_CP_exp:.4e}")
    print(f"  Relative Error:                           {abs(J_CP_theo - J_CP_exp) / J_CP_exp * 100:.2f}%")
    
    assert abs(J_CP_theo - J_CP_exp) < 1e-6, "Jarlskog invariant error!"
    print("  --> BATTERY 5 PASSED: Dimensionless J_CP dynamically calculated and verified against PDG.")
    return True
